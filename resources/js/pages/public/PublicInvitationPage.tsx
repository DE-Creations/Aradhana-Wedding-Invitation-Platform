...
const buildGoogleCalendarUrl = (wedding: WeddingData) => {
  ...
  const params = new URLSearchParams({
    action: "TEMPLATE",
    text: title,
    dates: `${formatGoogleCalendarDate(wedding.event_date, wedding.start_time)}/${formatGoogleCalendarDate(wedding.event_date, wedding.end_time)}`,
    details,
    location: `${wedding.venue_name}, ${wedding.venue_address}`,
    ctz: "Asia/Colombo",
  });
...
export const PublicInvitationPage = ({
  ...
  const googleCalendarLink = buildGoogleCalendarUrl(w);
      ...
      return (
        <>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="mb-10 text-center">
            ...
            <p className={`mt-4 text-sm uppercase tracking-[0.35em] ${selectedTypography.bodyFont} ${selectedTheme.subTextToneClassName}`}>{[w.event_date ? new Date(w.event_date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : '', w.venue_name].filter(Boolean).join(' · ')}</p>
          </motion.div>
        </>
      );
    }
  ...
  useEffect(() => {
    const diff = Math.ceil((new Date(w.event_date).getTime() - Date.now()) / (1000 * 60 * 60 * 24));
              ...
              <div className="mx-auto max-w-2xl">
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className={`mb-6 rounded-[1.4rem] p-6 ${selectedTheme.surfaceClassName}`}>
                  ...
                  <div className="mt-5 grid gap-4 md:grid-cols-3">
                    <div className="flex gap-3">
                      ...
                      <div>
                        <p className={`text-sm font-semibold ${selectedTypography.bodyFont}`}>{w.event_date}</p>
                        <p className={`text-xs ${selectedTypography.bodyFont} ${selectedTheme.subTextToneClassName}`}>Wedding Day</p>
                      </div>
                    </div>
                    <div className="flex gap-3">
                      <Clock className={`mt-0.5 h-5 w-5 shrink-0 ${selectedTheme.accentToneClassName}`} />
                      <div>
                        <p className={`text-sm font-semibold ${selectedTypography.bodyFont}`}>{w.start_time} - {w.end_time}</p>
                        <p className={`text-xs ${selectedTypography.bodyFont} ${selectedTheme.subTextToneClassName}`}>Poruwa at {w.poruwa_time}</p>
                      </div>
                    </div>
                    <div className="flex gap-3">
                      <MapPin className={`mt-0.5 h-5 w-5 shrink-0 ${selectedTheme.accentToneClassName}`} />
                      <div>
                        <p className={`text-sm font-semibold ${selectedTypography.bodyFont}`}>{w.venue_name}</p>
                        <p className={`text-xs ${selectedTypography.bodyFont} ${selectedTheme.subTextToneClassName}`}>{w.venue_address}</p>
                      </div>
                    </div>
                  </div>
                  <a href={w.google_maps_link} target="_blank" rel="noopener noreferrer" className={`mt-5 inline-flex rounded-full px-5 py-2 text-sm font-medium transition-opacity hover:opacity-90 ${selectedTheme.buttonClassName}`}>
                    View On Map
                  </a>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="text-center">
                  <button onClick={() => {
                    ...
                    <Heart className="mr-2 inline h-4 w-4 fill-current" /> RSVP Now
                  </button>
